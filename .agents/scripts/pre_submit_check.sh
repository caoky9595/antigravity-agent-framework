...
SECRET_HITS=$(grep -rn "api_key\s*=\s*['\"]" "$TARGET_DIR" --include="*.py" --include="*.ts" --include="*.js" 2>/dev/null | grep -v ".env" | grep -v "node_modules" | grep -v "venv" | grep -v "__pycache__" | grep -v "example" | grep -v "SKILL.md" || true)
...
SECRET_HITS2=$(grep -rn "password\s*=\s*['\"]" "$TARGET_DIR" --include="*.py" --include="*.ts" --include="*.js" 2>/dev/null | grep -v "node_modules" | grep -v "venv" | grep -v "test_" | grep -v "_test." | grep -v "example" | grep -v "SKILL.md" || true)
...
PRINT_HITS=$(grep -rn "^\s*print(" "$TARGET_DIR" --include="*.py" 2>/dev/null | grep -v "test_" | grep -v "_test." | grep -v "venv" | grep -v "__pycache__" | grep -v "scripts/" | grep -v "benchmark" | grep -v "SKILL.md" || true)
...
EXCEPT_HITS=$(grep -rn "except:" "$TARGET_DIR" --include="*.py" 2>/dev/null | grep -v "venv" | grep -v "__pycache__" | grep -v "# noqa" | grep -v "SKILL.md" || true)
...
MISSING_DOCS=0
for f in $(find "$TARGET_DIR" -name "*.py" -not -path "*/venv/*" -not -path "*/__pycache__/*" -not -path "*/node_modules/*" -not -path "*/test_*" -not -name "test_*" 2>/dev/null); do
    ...
    done < <(grep -n "^\s*def [a-zA-Z][a-zA-Z0-9_]*(" "$f" 2>/dev/null | grep -v "def _" || true)
done
...
MISSING_HINTS=0
for f in $(find "$TARGET_DIR" -name "*.py" -not -path "*/venv/*" -not -path "*/__pycache__/*" -not -path "*/node_modules/*" 2>/dev/null); do
...
TODO_HITS=$(grep -rn "TODO\|FIXME\|HACK\|XXX" "$TARGET_DIR" --include="*.py" --include="*.ts" --include="*.js" 2>/dev/null | grep -v "node_modules" | grep -v "venv" | grep -v "__pycache__" | grep -v "SKILL.md" | grep -v "KNOWLEDGE.md" || true)
if [ -n "$TODO_HITS" ]; then
    echo "$TODO_HITS"
