...
## 🏗️ 1. Cấu Trúc Đội Ngũ (8 Roles)

Hệ thống được chia thành 8 vai trò (Roles) cụ thể nằm trong thư mục `.agents/skills/`. Tùy vào độ phức tạp của Task, bạn có thể gọi đích danh một Role hoặc gọi Orchestrator để tự động điều phối toàn bộ.

1. **🎭 Orchestrator (Nhạc trưởng)**: Tiếp nhận yêu cầu phức tạp, phân rã công việc (decomposition) và điều phối các agent khác.
2. **🔍 Analyst (Phân tích & Lên kế hoạch)**: Sinh Repo Map, phân tích codebase, gom requirements và lập kế hoạch (Implementation Plan).
3. **📐 Architect (Kiến trúc sư)**: Thiết kế giải pháp (System Design, API Contracts) và viết ADR (Architecture Decision Record) cho các tính năng lớn.
4. **💻 Coder (Lập trình viên)**: Viết code chính xác theo plan. Tập trung vào Clean Code, Logging và Error Handling. Không tự ý đổi thiết kế.
5. **🛡️ Reviewer (Người kiểm duyệt)**: Review 2 vòng (Spec Compliance -> Code Quality). Dọn dẹp code rác, giữ codebase sạch sẽ.
6. **🧪 Tester (Kỹ sư kiểm thử)**: Đảm bảo nguyên tắc TDD. Viết test, chạy test và báo cáo. Chỉ khi test FAIL mới gọi Debugger.
7. **🐛 Debugger (Chuyên gia gỡ lỗi)**: Tìm Root Cause thay vì sửa triệu chứng. Ghi chép bài học vào Sổ tay phục hồi (Recovery Ledger).
8. **⚙️ DevOps**: Xử lý Docker, CI/CD pipelines, và Observability (Logging, Metrics).
...
### Trường hợp 1: Tính năng lớn, phức tạp (Full Pipeline)
> Hãy để Orchestrator làm mọi việc.
*   **Prompt của bạn:** *"Hãy dùng Orchestrator, tôi muốn thêm tính năng Đăng nhập bằng Google cho dự án."*
*   **Luồng chạy tự động:** Orchestrator -> Analyst -> Architect -> Coder -> Reviewer -> Tester.

### Trường hợp 2: Tính năng nhỏ, thêm một logic đơn giản
> Bỏ qua Architect để làm nhanh.
*   **Prompt của bạn:** *"Dùng Analyst và Coder: Hãy viết thêm một hàm utils để parse định dạng video sang dạng chuỗi."*
...
*   **Prompt của bạn:** *"Test ở file `test_video.py` đang fail. Gọi Debugger tìm Root Cause và fix lỗi này."*
